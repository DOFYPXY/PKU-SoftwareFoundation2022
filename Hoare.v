...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  unfold hoare_triple. intros. remember (H st). contradiction.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard, especially useful (hoare_asgn_wrong)
    ...
    When a = X + 1, the triple cannot been proved.
...
Proof.
  unfold hoare_triple.
  intros H.
  specialize H with (a:=X) (n:=0) (st:=empty_st) (st':= (Y!->3; X!-> 3)).
  assert (H0: empty_st =[ X:=3; Y:=X ]=> (Y!->3; X!->3)).
  {
    eapply E_Seq.
...
Proof.
  ...
  - assn_auto''. destruct H.
    unfold "<>" in H0. rewrite negb_true_iff in H0. 
    destruct H0. 
    rewrite <- H. lia.   *)
...
Proof.
  ...
  - apply hoare_havoc.
  - unfold havoc_pre. unfold "->>". unfold assn_sub. intros. 
    exists (aeval st X). simpl. rewrite t_update_shadow. rewrite t_update_same.
    apply H.
Qed. 
