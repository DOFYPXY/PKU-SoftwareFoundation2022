...
Proof.
  ...
  - eapply H_Consequence_pre. eapply H_Seq; eauto. intros. eauto.
  - eapply H_Consequence_pre. eapply H_If; eauto. intros. eauto.
  - eapply H_Consequence.     eapply H_While; eauto. intros. eauto. intros. eauto.
...
Proof.
  induction c; intros.
  - eapply H_Consequence_pre. apply H_Skip. intros. contradiction.
  - eapply H_Consequence_pre. apply H_Asgn. intros. contradiction.
  - eapply H_Consequence_pre. eapply H_Seq; eauto. intros. contradiction.
  - eapply H_If; eapply H_Consequence_pre; eauto; intros; destruct H; auto.
  - eapply H_Consequence. apply H_While with (P:= assert False).
    + eapply H_Consequence_pre. eauto. intros. destruct H. eauto.
    + eauto.
...
Proof.
  ...
  unfold wp. intros. apply H0. eapply E_WhileTrue.
  - assumption.
  - apply H.
  - assumption.
Qed.
...
Proof.
  ...
  {
    apply H_Consequence_pre with (P' := Q).
    - apply H_Skip.
    - eauto.
  }
  {
    ...
    - apply H_Asgn.
    - eauto.
    (* - intros. apply (HT st (x !-> aeval st a; st)). 
      + auto.
      + auto.   *)
  }
  {
    apply H_Seq with (P := P) (Q := (wp <{c2}> Q) ) (R := Q).
    (* apply wp_seq. *)
    {
      apply IHc1. 
      (* info_eauto. *)
      intros. unfold wp. intros. eapply HT. eapply E_Seq.
  ...
  {
    apply H_Consequence_post with (Q' := ((wp <{while b do c end}> Q) /\ ~ b)%assertion).
    ...
    - apply wp_is_weakest. unfold hoare_triple. apply HT.
    - intros. destruct H. unfold wp in H. apply H. 
      apply E_WhileFalse. apply Bool.not_true_iff_false. apply H0. 
  }
Qed.
