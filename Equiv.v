...
Proof.
  ...
  - apply while_true_nonterm with (c:=c) (st:=st) (st':=st') in Hb.
    contradiction.
  - assert (Htrue: bequiv <{true}> <{true}>). {unfold bequiv. auto. }
    apply while_true_nonterm with (c:=<{skip}>) (st:=st) (st':=st') in Htrue.
    contradiction.
Qed.
(** [] *)
...
Proof.
  intros.
  assert (Hx: cequiv <{x:=a}> <{x:=x}>).
  {
    unfold cequiv.
    intros. split; intros H0; inversion H0; subst; unfold aequiv in H.
    -  rewrite <- (H st). apply E_Asgn. reflexivity.
    -  rewrite -> (H st). apply E_Asgn. reflexivity.
  }
  split; intro Hc; inversion Hc; subst.
  - unfold cequiv in Hx. 
    eapply Hx. eapply identity_assignment. eassumption.
  - unfold cequiv in Hx. 
    eapply identity_assignment. eapply Hx. eassumption.
Qed.
(** [] *)
...
Definition equiv_classes : list (list com) :=
  [ [prog_a] ; [prog_b;prog_e] ; [prog_d] ; [prog_f;prog_g] ; [prog_c;prog_h] ;
[prog_i] ].
...
Proof.
  ...
  { unfold bequiv,cequiv.
    ...
    (*dofy: what does the induction on a H mean? *)
    + (* E_WhileFalse *)
...
Proof.
  assert (A: forall b b' c1 c1' c2 c2' st st',
             ...
             st =[ if b' then c1' else c2' end]=> st').
  {
    unfold bequiv. unfold cequiv. intros b b' c1 c1' c2 c2' st st' Hbe Hc1e Hc2e Hce.
    inversion Hce; subst.
    - apply E_IfTrue. 
      + eauto.
      + eapply Hc1e. apply H5. (*dofy: why dose eauto fail?*)
    - apply E_IfFalse.
      + eauto.
      + eapply Hc2e. apply H5.
  }
...
Proof.
  ...
  - (* while *)
    ...
    + auto using while_true.
    + auto using while_false.
Qed.
...
Proof.
  ...
  assert (H1 : ~ (empty_st =[ <{ while true do skip end }> ]=> empty_st)).
  {
    eapply while_true_nonterm.
    unfold bequiv. auto.
  }
  assert (H2 : empty_st =[ <{ skip }> ]=> empty_st).
  {
    apply E_Skip.
  }
...
Proof. 
  left. unfold cequiv.
  destruct (eqb_string X Y) eqn:E.  (*dofy: other simpler solution?*)
  {
    assert (Heq: pXY = pYX).
    {
      unfold pXY, pYX. 
