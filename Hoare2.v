...
(** **** Exercise: 3 stars, standard (factorial)
  ...
  do   {{ Y * X! = m! /\ ~(X = 0)              }} ->>  (a - OK)
    ...
    Condition (a) is able to be proof by using lemma that "forall x>0, x! = x * (x-1)!".
...
(**
  ...
  {{ (min X Y) + Z = (min a b)             }}
  while ~(X = 0) && ~(Y = 0) do
    {{ (min X Y) + Z = (min a b) /\ (~(X = 0) && ~(Y = 0)) }} ->> (a - OK)
  ...
  {{ (min X Y) + Z = (min a b) /\ (X = 0 || Y = 0) }} ->> (b - OK)
  {{ Z = min a b }}

  Condition (a) is able to be proof by using lemma2.
    Condition (b) is obvious.
*)
